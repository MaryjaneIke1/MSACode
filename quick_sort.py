...
def quick_sort(arr):
    ...
    #Inside While loop. Loop runs until stack of partitions is enmpty 
    while stack:
        #Get the next partiotn to process
        low, high = stack.pop()
        #Partition the array. Find the pivot index for that partition. Call the partition function
        pivot_index = partition(arr, low, high)
        #if there are items on teh left of the pivot, putthem on the stack 
...
def partition(arr, low, high):
    ...
    i = low - 1
    #loop through the partition to place all itmes <= the pivot to the left. And >= pivot to the right
