...
def dijkstra(graph, start):
    ...
    tentative = {}
    confirmed = {}
    #add start node cost to teh confirmed dictionary 
    confirmed[start] = 0
    #place the start nodes neghbors in the tentative dictionary
    ...
    while tentative:
        ...
        #select nodes from tentative dictionary with the lowest cost
        for node, distance in tentative.items():
            #determin shortest distance node
        ...
        for node, distance in graph[next].items():
            ...
            #if the neighhbor is not in tentative, add to tentative.
